...
import { Data, StudentLevels } from "../utils/types";
...
// const response: Data<StudentLevels[]> = {
//   data: [
//     {
//       semester: 1,
//       year: 2023,
...
const topics: string[] = ["grammar", "listening & speaking", "writing"];
...
function Classes() {
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useGetStudentLevelsQuery({ run: 12345678 });
  ...
  //   const data = response.data.reduce((acc, { year, semester, levelName }) => {
  //     if (!acc[levelName]) {
  //       acc[levelName] = {};
  //     }
  ...
  //       acc[levelName][year] = {};
  //     }
  //     if (!acc[levelName][year][semester]) {
  //       acc[levelName][year][semester] = [];
  //     }
  //     return acc;
  //   }, {});
  ...
  return (
    <>
      ...
      {(isLoading || isFetching) && (
        <ThreeDots className="threeDots" fill="#2F4858" />
      )}
      {response && (
                    ...
                    >
                      ...
                      <IoIosArrowDown
                        className={`ioIosArrowDown ${
                          isLevelSelect ? "rotate" : ""
                        }`}
                      />
                    </div>
                    {response.data.map(
                      ({ year, semester, levelName: lName }) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {response.data.map(({ year: y, semester }) => {
                                if (isLevel && year === y) {
                                  const isSemesterSelect =
                                    semester === semesterYear.semester;
                                  return (
                                    <div
                                      key={`${levelName}-${year}-${semester}`}
                                      className={`dropdown ${
                                        isSemesterSelect ? "expand" : ""
                                      }`}
                                    >
                                      <div
                                        className={`flex ${
                                          isSemesterSelect ? "active" : ""
                                        }`}
                                        onClick={() =>
                                          handlerClickSemester(semester)
                                        }
                                      >
                                        <p>
                                          {t("semester")} {semester}
          ...
          <section className="class-list">
            {topic !== "" && (
              <table className="classes-table">
                <thead>
                  <tr>
                    <td colSpan={5}>Lista de clases</td>
                  </tr>
                  <tr>
                    <td>Semana</td>
                    <td>Contenido</td>
