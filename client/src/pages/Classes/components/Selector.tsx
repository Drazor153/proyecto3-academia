	...
	return (
		<div className={`dropdown ${isLevelSelect ? 'expand' : ''}`}>
			...
			>
				<p>{t(level)}</p>
				<IoIosArrowDown
					className={`ioIosArrowDown ${isLevelSelect ? 'rotate' : ''}`}
				/>
			</div>
			{children}
		</div>
	...
	return (
		<div className={`dropdown ${isYearSelect ? 'expand' : ''}`}>
			...
			>
				<p>{year}</p>
				<IoIosArrowDown
					className={`ioIosArrowDown ${isYearSelect ? 'rotate' : ''}`}
				/>
			</div>
			{children}
		</div>
...
export function Semester({
	semester,
	isSemesterSelect,
	handlerClickSemester,
	children,
}: SemesterProps) {
	return (
		<div className={`dropdown ${isSemesterSelect ? 'expand' : ''}`}>
			<div
			...
			>
				...
				<IoIosArrowDown
					className={`ioIosArrowDown ${isSemesterSelect ? 'rotate' : ''}`}
				/>
			</div>
			{children}
		</div>
...
interface LessonProps {
	...
	handlerClickLesson: ({ lesson }: { lesson: string }) => void;
	children?: React.ReactNode;
}
...
export function Lesson({
	...
	return (
		<div
			...
			onClick={() =>
				handlerClickLesson({ lesson: `${lesson.id} ${lesson.lesson}` })
			}
		>
			{children}
		</div>
...
export default function Selector({ role, select, setSelect }: SelectorProps) {
	const {
		data: levels,
	...
	if (error) toast.error(t('error_loading_levels'));
											...
											return (
												...
												>
													{lessons.map(({ id, lesson: letter }) => {
														...
														const isLessonSelect =
															lesson.id === select.lesson.id;
														return (
															<Lesson
