...
import { ClassesStudent, ClassesTeacher, Data, PostClass } from "../../../utils/types"

import { BiSolidPlusSquare } from "react-icons/bi";
import { ImEyeMinus, ImEyePlus } from "react-icons/im";
import { IoIosClose } from "react-icons/io";
import { MdOutlineDeleteForever } from "react-icons/md";
import { useAddClassMutation, useDeleteClassMutation, useGetClassesByLessonIdQuery, useGetStudentsByLessonQuery, useUpdateClassMutation } from "../../../redux/services/classesApi";
import { Select } from "../types";
import { RutFormat, deconstructRut, formatRut } from "@fdograph/rut-utilities";



type AttendanceList = { student: { run: string, name: string, first_surname: string }, attendance: string };
...
function ClassesTable({ role, select, run }: ClassTableProps) {

    const { data: classes, isLoading: isLoadingClasses } = useGetClassesByLessonIdQuery({ role: role, lesson: select.lesson.id, run: role === 'students' ? run : undefined });
    ...
    return (
        <>
            ...
            >
                ...
                <tbody>
                    {
                        (classes.data as ClassesStudent[]).map(({ attendance, contents, teacher, week }) => {
                            ...
                            return (
                                <tr key={week} className="grid">
                                    ...
                                    <td datatype="teacher">{name} {first_surname}</td>
                                    <td>
...
function TableTeacher({ classes, select }: TableProps & { select: Select }) {
    ...
    const [showModalDeleteClass, setShowModalDeleteClass] = useState<boolean>(false)


    const { data: students, isLoading: isLoadingStudents } = useGetStudentsByLessonQuery({ lesson: select.lesson.id });
    ...
    const handlerClickViewClass = ({ id }: { id: number }) => {
        ...
        const { attendance, contents, week } = (classes!.data as ClassesTeacher[]).find(({ id: _id }) => _id === id)!;

        attendance.map(({ run, dv, name, attended, first_surname }) => {
            attendanceList.push({ student: { run: formatRut(`${run}-${dv}`, RutFormat.DOTS_DASH), name, first_surname }, attendance: attended ? "present" : "absent" });
        });
            ...
            }
            {
                ...
                <>
                    ...
                    >
                        ...
                        <tbody>
                            {
                                (classes.data as ClassesTeacher[]).map(({ id, attendance, contents, week }) => {
                                    const contentsList = contents.split(',')
                                    const presents = attendance.filter(({ attended }) => attended).length;
                                    ...
                                    return (
                                        <tr key={week} className="grid">
                                            ...
                                            <td>
                                                {`${presents} / ${total} (${Math.round(presents / total * 100)}%)`}
                                            </td>
                                            <td>
                                                {((!showModalViewClass) || selectedClass.week !== week) && (
                                                    <>
                                                        <ImEyePlus onClick={() => handlerClickViewClass({ id: id })} className="imEyePlus" />
                                                    </>
                                                )}
                                                {
                                                    showModalViewClass && selectedClass.week === week && (<ImEyeMinus className="imEyeMinus" />)
                                                }
                                                <MdOutlineDeleteForever onClick={() => handlerClickDelete({ id })} className="mdOutlineDeleteForever" />
                                            </td>
                                        </tr>
                                    );
                                })
...
function ModalClassList({ showModalViewClass, selectedClass, setSelectedClass, setShowModalViewClass }: ModalClassListProps) {
    ...
    const setPositionTable = () => {
        const table = document.querySelector('.table-attendance-list') as HTMLTableElement;
    ...
    const handlerClickConfirm = () => {
        ...
        const body: PostClass = {
            ...
            attendance: selectedClass.attendanceList.map(({ attendance, student }) => {
    ...
    return (
        <>
            <div className={`background ${showModalViewClass ? 'showModalViewClass' : ''}`} />
            <div className={`attendance ${showModalViewClass ? 'showModalViewClass' : ''}`}>
                ...
                <div className="body">
                    ...
                    <div className="lesson">
                        <p>{t('lesson')}</p>
                        <input type="text" name="lesson" id="lesson" value={selectedClass.lesson.lesson} readOnly />
                    </div>
                            ...
                            {selectedClass.attendanceList.map(({ attendance, student }) => {
                                return (
                                    <tr key={`${student.run} ${attendance}`} className="student">
                                        ...
                                        <td>
                                            <p>{student.first_surname}, {student.name}</p>
                                        </td>
                                        <td>
                                            <p>{t('present')}</p>
                                            <input type="checkbox" name={`attendance ${student.name}`} id={`attendance ${student.name}`} checked={attendance === 'present'}
                                            ...
                                            />
                                            <label className="label-attendance" htmlFor={`attendance ${student.name}`}></label>
                                        </td>
                                    </tr>
                                );
                            })}
...
function ModalDeleteClass({ selectedClass, showModalDeleteClass, setShowModalDeleteClass }: ModalDeleteClassProps) {
    const [deleteClasses] = useDeleteClassMutation();

    const handlerClickConfirm = () => {
        deleteClasses({ id: selectedClass.id }).unwrap().then((res) => {
            console.log(res)
            setShowModalDeleteClass(!showModalDeleteClass)
        });
    }
    ...
    return (
        <>
            <div className={`background ${showModalDeleteClass ? 'showModalDeleteClass' : ''}`} />
            <div className={`delete ${showModalDeleteClass ? 'showModalDeleteClass' : ''}`}>
                <h2>{t('delete_class')}</h2>
                <IoIosClose className="ioClose" onClick={() => setShowModalDeleteClass(!showModalDeleteClass)} />
                <div className="body">
